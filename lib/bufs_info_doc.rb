...
module DataStoreModels
  ...
  class FilesMgr
    ...
    def add_files(node, file_datas)
      ...
      puts "FIlesMgr operating on node: #{node.inspect}"
      puts "FIlesMgr rec id: #{record['_id'].inspect}"
      if node.respond_to? :attachment_doc_id
        ...
        #val = node.attachment_doc_id
        #puts "FilesMgr Atts Val current: #{val.inspect}"
        #val << record['_id']
        #node.attachment_doc_id = val
      else
        node.iv_set(:attachment_doc_id,  record['_id'] )
        puts "FilesMgr added att id: #{node.attachment_doc_id}"
      end
      #record[_id]
    ...
    def subtract_some(node, model_basenames, bia_class)
      if node.attachment_doc_id
        #raise node.attachment_doc_id.inspect
        puts "Node Attachment ID: #{node.attachment_doc_id.inspect}"
        puts "BIA Class: #{bia_class.inspect}"
        bia_doc = bia_class.get(node.attachment_doc_id)
        puts "BIA Doc: #{bia_doc.inspect}"
...
=begin
    ...
    @namespace = BufsInfoDocEnvMethods.set_namespace(@db, db_user_id)
    BufsInfoDocEnvMethods.set_view_all(@db, @design_doc, @collection_namespace)
    @files_mgr = DataStoreModels::CouchRest::FilesMgr.new(:attachment_actor_class => self.user_attachClass)
    @views_mgr = DataStoreModels::CouchRest::ViewsMgr.new(:db => @db, :design_doc => @design_doc)
    return @namespace 
=end
  end
  ...
  #  DataStoreModels::CouchRest::MetadataKeys
  #end
  
  #def self.files_mgr
  #  self.class_env.files_mgr
  #end
  ...
  def save
    ...
    #existing_doc_id = DataStoreModels::CouchRest.generate_model_key(@collection_namespace, node_id)
    #existing_doc = self.class.class_env.db.get(existing_doc_id)
